...
class AuthenController extends ControllerBase{
  ...
  public function initialize()  //ตัวเริ่ม*
    {
      parent::initialize();  //เรียก func class แม่มา
	  $this->view->disableLevel(View::LEVEL_MAIN_LAYOUT); //not show view ในเลเวลบน
	  $this->view->setTemplateAfter('login'); //show template ตัวล็อกอิน
      ...
      if($member){
        if($member->active == '1'){
          if($this->security->checkHash($pass, $member->password)){ // ตรวจสอบรหัสด้วย key การเข้ารหัส
            $this->session->set('memberAuthen', $member->id); // กำหนด session =$_SESSION['memberAuthen']=$member->id;
  ...
  public function signUpAction(){
    if($this->request->isPost()){
      ...
      $member->save();
      $this->response->redirect('authen'); //หลังจากเซฟแล้ว จะให้ไปที่หน้า controller authen
